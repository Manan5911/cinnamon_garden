...
import 'package:booking_management_app/core/screens/kitchen/view_booking_screen.dart';
...
class _KitchenDashboardState extends ConsumerState<KitchenDashboard>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      // Step 1: Get the logged-in kitchen user’s restaurantId
      final userDoc = await FirebaseFirestore.instance
          .collection('users')
          .doc(user.uid)
          .get();

      final String? restaurantId = userDoc.data()?['restaurantId'];

      if (restaurantId == null) {
        SnackbarHelper.show(
          context,
          message: 'No restaurant assigned to this user.',
          type: MessageType.error,
        );
        return;
      }

      // Step 2: Load all bookings
      await controller.loadAllBookings();

      // Step 3: Apply restaurant filter and date range
      controller.applyCustomFilters(restaurantIds: [restaurantId]);
      controller.filterByDateRange(_selectedRange!);

      // Step 4: Load restaurant/manager map for display
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            _selectedRange == null
                                ? const SizedBox.shrink() // don’t show anything while range is null
                                : Text(
                      ...
                      Container(
                        decoration: BoxDecoration(
                          ...
                          shape: BoxShape
                              .circle, // use BoxShape.rectangle to allow borderRadius // 🎯 rounded corners
                        ),
